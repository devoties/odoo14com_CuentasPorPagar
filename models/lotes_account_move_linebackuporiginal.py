...
#este es compute
    ...
    busqueda_uno = fields.Integer(string='Qty Contratos Vig',compute='get_contracts_data')
    ...
    def get_aditional_data(self):
        for line in self:
            ...
            line.es_opinion = line.env['opinion_cumplimiento_sat'].search_count([('opinion_cumplimiento_partner_rel','=',line.data_rel.partner_id.id)])
            #calcula qty de ines vigentes o vencidas
            calcular_ines = line.env['ine_sat'].search_count([('ine_partner_rel','=',line.data_rel.partner_id.id),('fecha_vencimiento','>=',date.today())])
            if calcular_ines >= 1:
                line.es_ine = True
            if calcular_ines < 1:
                line.es_ine = False
            #calcula las caratulas que estan el rango de fecha de la factura relacionada "data_rel"
            line.es_cif = line.env['cif'].search_count([('cif_rel', '=', line.data_rel.partner_id.id)])
    ...
    def get_contracts_data(self):
        for line in self:
            line.busqueda_uno = line.env["huertas_contratos_terceros"].search_count([('huertas_contratos_terceros_huertas_rel', '=', line.name.sader.id),('fecha_vencimiento','>=',date.today())])
            if line.busqueda_uno >= 1:
